...
###MATLABCODE 
    ...
    def voccalculator_run_onoffprofile(self):
        ...
        # # Cooling OFF
            ...
            print(self.stop_onoff_conc_run)
            while not self.stop_onoff_conc_run:
                print(self.stop_onoff_conc_run, "ON STATE")
                if time.time() - pulse_starttime >= run_time:
                    break
                
                pulseon_start = time.time()
                while time.time() - pulseon_start < on_time: 
                    print("time that it is on", time.time() - pulseon_start, "ontime", on_time)
                    # ON state
                ...
                print(self.stop_onoff_conc_run, "OFF STATE")
                pulseoff_start = time.time()
                print(pulseoff_start)
                while time.time() - pulseoff_start < off_time: 
                    print("time that it is off", time.time() - pulseoff_start, "offtime", off_time)
                    # OFF state
                    self.mfcs[0].set_massflow(0)
